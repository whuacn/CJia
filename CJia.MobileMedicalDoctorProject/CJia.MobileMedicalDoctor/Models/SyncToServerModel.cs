...
namespace CJia.MobileMedicalDoctor.Models
{
    public class SyncToServerModel : ModelBase
    {
        ...
        {
            if (ModelBase.isBusy) return false;
            ...
            switch (sl.TableName)
            {
                ...
                case "[PatientCheckImages]": Result = cmd.ExecuteQuery<Data.PatientCheckImages>().SingleOrDefault(); break;
            ...
            if (Result != null)
            {
                ...
                object SyncResult = await RemoteExcute(new object[] { sl.TableName, sl.KeyField, sl.KeyValue, sl.ChangeType, sl.ChangeDate, Fields, Data });
