...
namespace CJia.Net.Data
{
    ...
    /// SQL Server数据访问组件
    /// </summary>
    public class SqlAdapter: IDataClient
    {
        /// <summary>
        /// SQL Server数据访问组件
        /// </summary>
        ...
        /// <param name="TransID"></param>
        /// <returns></returns>
        SqlTransaction GetTransaction(string TransID)
        {
            if (TransID.Length == 0) return null;
            return dictTransaction[TransID];
        }
        #endregion

        #region Connection
        Dictionary<string, SqlConnection> dictConnection = new Dictionary<string, SqlConnection>();
        Dictionary<string, SqlTransaction> dictTransaction = new Dictionary<string, SqlTransaction>();
        /// <summary>
        /// 获取Oracle连接
        /// </summary>
        /// <param name="TransID">事务ID</param>
        ...
        /// <returns></returns>
        SqlConnection GetSqlConnection(string TransID, string StrConn)
        {
            if (TransID.Length == 0)
            {
            ...
            {
                transConn = new SqlConnection(StrConn);
                transConn.Open();
                SqlTransaction dbTrans = transConn.BeginTransaction(IsolationLevel.ReadCommitted);
                lock (dictConnection)
                {
        ...
        /// </summary>
        /// <param name="TransID">事务ID</param>
        /// <param name="sqlConnection">Oracle连接</param>
        void CloseConnection(string TransID, SqlConnection sqlConnection)
        {
            if (TransID.Length == 0 && sqlConnection != null)
            {
                sqlConnection.Close();
        ...
        public string QueryScalar(string TransID, string StrConn, string SqlText, object[] SqlParams)
        {
            ...
            try
            {
                using (SqlCommand oCmd = new SqlCommand(SqlText, oConn))
                {
        ...
        public DbDataReader QueryPaging(string TransID, string StrConn, string SqlText, object[] SqlParams)
        {
            ...
            try
            {
                SqlCommand oCmd = new SqlCommand(SqlText, oConn);
                CreateSqlParam(oCmd, SqlParams);
        ...
        public DbDataReader QueryProcedurePaging(string TransID, string StrConn, string ProcedureName, Dictionary<string, object> Params)
        {
            ...
            try
            {
                SqlCommand oCmd = new SqlCommand(ProcedureName, oConn);
                oCmd.CommandType = CommandType.StoredProcedure;
                CreateSqlProcedureParam(oCmd, Params);
                SqlDataReader odr = oCmd.ExecuteReader(CommandBehavior.CloseConnection);
                return odr;
        ...
        private void CreateSqlParam(SqlCommand oCmd, object[] SqlParams)
        {
        ...
        /// <param name="oCmd"></param>
        /// <param name="Params"></param>
        private void CreateSqlProcedureParam(SqlCommand oCmd, Dictionary<string, object> Params)
        {
        ...
        /// <param name="oCmd"></param>
        /// <param name="Params"></param>
        private void ReturnSqlProcedureOutputParam(SqlCommand oCmd, ref Dictionary<string, object> Params)
        {
            if(Params == null || Params.Count == 0)
                return;
