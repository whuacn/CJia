...
namespace CJia.Net
{
    ...
    /// </summary>
    internal class CJiaServer : IServerApplication
    {
        ...
        /// </summary>
        private readonly IServer _scsServer;
        ...
        public CJiaServer(IServer scsServer)
        {
            if (scsServer == null)
            {
        ...
        public void AddService<TServiceInterface, TServiceClass>(TServiceClass service) 
            where TServiceClass : CJiaService, TServiceInterface
        ...
        private void Client_MessageReceived(object sender, MessageEventArgs e)
        {
            ...
            try
            {
                ...
                {
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new CJiaRemoteException("There is no service with name '" + invokeMessage.ServiceClassName + "'"));
                    return;
                }
                ...
                catch (TargetInvocationException ex)
                {
                    ...
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new CJiaRemoteException(innerEx.Message + Environment.NewLine + "Service Version: " + serviceObject.ServiceAttribute.Version, innerEx));
                    return;
                }
                catch (Exception ex)
                {
                    SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new CJiaRemoteException(ex.Message + Environment.NewLine + "Service Version: " + serviceObject.ServiceAttribute.Version, ex));
                    return;
                }
            }
            catch (Exception ex)
            {
                SendInvokeResponse(requestReplyMessenger, invokeMessage, null, new CJiaRemoteException("An error occured during remote service method call.", ex));
                return;
            }
        }
        ...
        private sealed class ServiceObject
        {
            ...
            /// </summary>
            public CJiaService Service { get; private set; }

            /// <summary>
            /// ScsService attribute of Service object's class.
            /// </summary>
            public CJiaServiceAttribute ServiceAttribute { get; private set; }
            ...
            public ServiceObject(Type serviceInterfaceType, CJiaService service)
            {
                Service = service;
                var classAttributes = serviceInterfaceType.GetCustomAttributes(typeof(CJiaServiceAttribute), true);
                if (classAttributes.Length <= 0)
                {
                    throw new Exception("Service interface (" + serviceInterfaceType.Name + ") must has ScsService attribute.");
                }

                ServiceAttribute = classAttributes[0] as CJiaServiceAttribute;
    ...
    /// </summary>
    public static class CJiaServerBuilder
    {
        /// <summary>
