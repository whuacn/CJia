...
namespace CJia.Net.Client
{
    ...
    internal abstract class ClientBase : IClient
    {
        ...
        bool isAllowAutoPing = true;
        /// <summary>
        /// 是否允许自动Ping远程服务器以检测连接是否中断，默认30秒ping一次
        /// </summary>
        public bool AllowAutoPing
        {
            get { return isAllowAutoPing; }
            set { isAllowAutoPing = value; }
        }
        ...
        private ICJiaWireProtocol _wireProtocol;
        ...
        {
            if (e.Message is CJiaPingMessage)
            {
                return;
            }
        ...
        {
            if (!isAllowAutoPing) return;
            _pingTimer.Period = pingInterval;
            if (CommunicationState != CommunicationStates.Connected)
            {
                return;
            }
            ...
            try
            {
                ...
                _communicationChannel.SendMessage(new CJiaPingMessage());
            }
            catch
            {
        ...
        protected virtual void OnMessageReceived(ICJiaMessage message)
        {
