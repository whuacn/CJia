    ...
    public partial class MergeView : CJia.Health.Tools.View, CJia.Health.Views.IImagesInputView
    {
        ...
        void LURecordNO_SelectValueChanged(object sender, EventArgs e)
        {
            if (RecordNOData != null && LURecordNO.DisplayText.Length != 0)
            {
                ...
                {
                    MessageBox.Show("此病案为已提交审核，不能进行病案合并");
                    BindNull();
                    return;
                }
                if (checkState == "101")//审核通过
                {
                    MessageBox.Show("此病案为审核通过，不能进行病案合并");
                    BindNull();
                    return;
                }
        ...
        private void btnMerge_Click(object sender, EventArgs e)
        {
            if (pictureView.GetFocusedDataRow() != null && PictureInfo != null && PictureInfo.Rows.Count > 0)
            {
                ...
                try
                {
                    foreach (DataRow dr in PictureInfo.Rows)
                    {
                        ...
                        if (dr["Pic_Name"].ToString().Substring(0, 2) == "KB")//为空白页
                        {
                            ...
                            //    {
                            //        SMpageList.Add(dr["Pic_Page"].ToString());//把空白病案的页码存起来
                            //    }
                            //}
                            //else //创佳
                            //{
                ...
                {
                    MessageBox.Show("病案不存在或已删除，请与管理员联系。。。");
                    waitUC.ParentForm.Close();
                    this.Enabled = true;
                }
                ...
                {
                    if (Message.ShowQuery("只存在扫描的病案,是否继续合并?", Message.Button.YesNo) == Message.Result.Yes)
                    {
                        Merge("104");
                        isModify = true;
                    }
                ...
                else if (SMpageList.Count == 0 && PZpageList.Count != 0)
                {
                    ...
                    {
                        MessageBox.Show("拍照的病案与空白页不符，不能继续合并。");
                        isModify = false;
                    }
                    else
                    {
                        if (Message.ShowQuery("只存在拍照的病案,是否继续合并?", Message.Button.YesNo) == Message.Result.Yes)
                        {
                            foreach (DataRow dr in PictureInfo.Rows)
                            {
                                if (dr["Pic_Name"].ToString().Substring(0, 2) == "PZ")
                    ...
                    for (int i = 0; i < SMpageList.Count; i++)
                    {
                        foreach (DataRow dr in PictureInfo.Rows)
                        {
                            if (dr["Pic_Name"].ToString().Substring(0, 2) == "KB")
                            {
                                ...
                                {
                                    //File.Delete(dr["Pic_Path"].ToString());//删除空白病案
                ...
                else
                {
                    ...
                    lblMesg.Text = str;
                    if (Message.ShowQuery("空白病案张数与拍照病案页码张数不符，是否对此份病案重新录入？空白病案页码为" + str + "", Message.Button.YesNo) == Message.Result.Yes)
                    {
                        Merge("107");
                    }
                    //MessageBox.Show("空白病案张数与拍照病案页码张数不符，合并失败，请对此份病案重新录入");
                    //Merge("107");
                    //DataTable data = CreatePictureDate();//重新加载病案
        ...
        public bool isBlankPage(Bitmap bp, int intHeight)
        {
            ...
            if (isjjfb == "0")//妇保
            {
                ...
                if (f < 0.985)//空白部分超过99%就认为此病案为空白病案
                {
                    bol = false;
                }
                return bol;
            ...
            {   //创佳
                ...
                if (f < blank)//空白部分超过99%就认为此病案为空白病案
                {
                    bol = false;
                }
                return bol;
