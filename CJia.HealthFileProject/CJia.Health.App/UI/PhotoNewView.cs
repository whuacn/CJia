...
{
    public partial class PhotoNewView : CJia.Health.Tools.View, CJia.Health.Views.IImagesInputView
    {
        ...
        public void BindParentFormBtn()
        {
            if (this.ParentForm != null)
            {
                CJia.Controls.CJiaTabControl tab = (CJia.Controls.CJiaTabControl)this.Parent.Parent;
                DevExpress.XtraTab.XtraTabPage page = (DevExpress.XtraTab.XtraTabPage)this.Parent;
                if (tab.SelectedTabPage == page)
                {
        ...
        private void ckModel_CheckedChanged(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                axCmCaptureOcx1.Visible = false;
                axCmCaptureOcx1.StopPreview();//停止拍摄
                btnTakePhoto.Enabled = false;
        ...
        {
            if (ckZiDong.Checked)
                axCmCaptureOcx1.EnableAuto_Range(1);
            else
                axCmCaptureOcx1.EnableAuto_Range(0);
        }
        ...
        public bool isSuccessCheckStatus(string checkState, bool bol)
        {
            ...
            {
                MessageBox.Show("此病案为已提交审核，不能进行病案拍照上传");
                BindNull();
                return false;
            }
            if (checkState == "101")
            {
                MessageBox.Show("此病案为已审核通过，不能进行病案拍照上传");
                BindNull();
                return false;
            }
            if (checkState == "104")
            {
                MessageBox.Show("此病案为已合并，不能进行病案拍照上传");
                BindNull();
                return false;
            }
            ...
            {
                MessageBox.Show("此病案为审核未通过，请查看已入库病案的审核原因，然后进行重新拍照上传");
            }
            return true;
        }
        ...
        public void InitHighshotMeter()
        {
            ...
            //int devnum = axCmCaptureOcx1.GetDeviceNum();
            //if (devnum == 0)
            //    MessageBox.Show("没法发现视频设备");
            axCmCaptureOcx1.BuildVideo();
            axCmCaptureOcx1.StartPreview();
            int JQ = int.Parse(CJia.Health.Tools.ConfigHelper.GetAppStrings("JQ"));
            axCmCaptureOcx1.SetJpgQuality(JQ);
            int DPI = int.Parse(CJia.Health.Tools.ConfigHelper.GetAppStrings("DPI"));
            axCmCaptureOcx1.SetPicDpi(DPI);
            axCmCaptureOcx1.VideoFit();
            axCmCaptureOcx1.EnableAuto_Range(1);
        }
        /// <summary>
        /// 拍照
        ...
        public void CaptureImage()
        {
            ...
            if (isHave)
            {
                ...
                {
                    axCmCaptureOcx1.TakePicture(filePath);         //拍照保存病案
                    DataTable data = CreatePictureDate(txtFolder.Text);
                    pictureGrid.DataSource = data;
                    pictureView.FocusedRowHandle = data.Rows.Count - 1;
                    PageIdentity();
