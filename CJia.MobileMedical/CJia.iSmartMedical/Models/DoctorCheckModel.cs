...
namespace CJia.iSmartMedical.Models
{
    public class DoctorCheckModel : ModelBase, IModel
    {
        ...
        /// <param name="InhosID"></param>
        /// <returns></returns>
        public DoctorCheckLog QueryLocalCheckLog(string DCLID)
        {
            using (SQLite.SQLiteConnection conn = new SQLite.SQLiteConnection(iDB.DbFile))
            {
                string SqlText = @"Select * From [DoctorCheckLog] Where[DCLID]=?";
                SQLite.SQLiteCommand cmd = conn.CreateCommand(SqlText, DCLID);
                var Result = cmd.ExecuteQuery<DoctorCheckLog>();
