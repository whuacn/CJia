...
namespace CJia.iSmartMedical.Win8.Doctor
{
    ...
    public sealed partial class CheckLogDetailPage : BasePage, Views.ICheckLogDetailPageView
    {
        ...
        public CheckLogDetailPage()
        {
            ...
            iCommon.CheckLogSyncComplet += iCommon_CheckLogSyncComplet;
        }

        public event EventHandler<Views.CheckLogDetailEventArgs> OnUpdateCheckLog;
        ...
        void iCommon_CheckLogSyncComplet(object sender, CheckLogSyncEventArgs e)
        {
            for (int i = 0; i < e.InhosID.Count; i++)
            {
                ...
                {//判断是否有当前病人的更新
                    ...
                    {
                        if (e.DCLID[c] == CurrentDCLID) continue;
        ...
        {
            if (IsImageChanged && clDetail.CheckLog != null)
            {
        ...
        private void ShowToolColor(Button button)
        {
            ...
            {
                btn.BorderBrush = UI.NewSolidColorBrush("#FF007ACC"); //new SolidColorBrush(Colors.White);
            }
        ...
        {
            SetImage(ImageData, false);
        }
        private async void SetImage(byte[] ImageData, bool IsUpdate)
        {
            if (!IsUpdate) MedicalCanvas.Children.Clear();
            if (ImageData == null)
            {
        ...
        private void btnAllLog_Click(object sender, RoutedEventArgs e)
        {
            if (mmSZoom.Visibility == Windows.UI.Xaml.Visibility.Collapsed)
                mmSZoom.Visibility = Windows.UI.Xaml.Visibility.Visible;
            else
                mmSZoom.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
        }
        ...
        private void MedicalCanvas_Tapped(object sender, TappedRoutedEventArgs e)
        {
            this.mmSZoom.Visibility = Windows.UI.Xaml.Visibility.Collapsed;
        }


        public void ExeSaveSuccess()
        {
