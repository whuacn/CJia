...
{
    public partial class PharmFilterView : XtraUserControl
    {
        ...
        public void BindData(DataTable Pharm, string illfieldId)
        {
            this.illfieldId = illfieldId;
            if(Pharm != null && Pharm.Rows != null && Pharm.Rows.Count > 0)
            {
                DataColumn isCheck = new DataColumn("ISCHECK", typeof(System.Boolean));
                Pharm.Columns.Add(isCheck);
                DataColumn initCheck = new DataColumn("INIT_CHECK", typeof(System.Boolean));
                Pharm.Columns.Add(initCheck);
                for(int i = 0; i < Pharm.Rows.Count; i++)
                {
                    if(Pharm.Rows[i]["FILTER_PHARM"].ToString() == "0")
                    {
                        Pharm.Rows[i]["ISCHECK"] = false;
                        Pharm.Rows[i]["INIT_CHECK"] = false;
                    }
                    else
                    {
                        Pharm.Rows[i]["ISCHECK"] = true;
                        Pharm.Rows[i]["INIT_CHECK"] = true;
                    }
                }
            }
        ...
        void isCheck_CheckedChanged(object sender, System.EventArgs e)
        {
            DevExpress.XtraEditors.CheckEdit rce = sender as DevExpress.XtraEditors.CheckEdit;
            DataRow selectRow = this.gvPatient.GetFocusedDataRow();
            if(this.illfieldId != "0")
            {
                if(rce.Checked)
                {
                    this.deletePharmFilter(this.illfieldId, selectRow["PHARM_ID"].ToString());
                }
                else
                {
                    this.addPharmFilter(this.illfieldId, selectRow["PHARM_ID"].ToString());
                }
            }
            else
            {
                selectRow["INIT_CHECK"] = true;
                rce.Checked = true;
            }
        }

        public event CJia.PIVAS.Tools.Delegate.NoResTwoPar addPharmFilter;
        public event CJia.PIVAS.Tools.Delegate.NoResTwoPar deletePharmFilter;
        ...
        private void txtPharmName_TextChanged(object sender, EventArgs e)
        {
            ...
            if(data != null && data.Rows != null && data.Rows.Count > 0)
            {
                ...
                {
                    int start = data.Rows[i]["FLAG"].ToString().ToUpper().IndexOf(this.txtPharmName.Text.Trim().ToUpper());
                    if(start != -1)
                    {
