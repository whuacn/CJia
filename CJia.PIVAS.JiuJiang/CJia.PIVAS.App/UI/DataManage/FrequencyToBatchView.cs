...
namespace CJia.PIVAS.App.UI.DataManage
{
    ...
    public partial class FrequencyToBatchView : CJia.PIVAS.Tools.View, CJia.PIVAS.Views.DataManage.IFrequencyToBatchView
    {
        ...
        private void Init()
        {
            ...
            //    this.gridView1.DoubleClick -= BtnUpdate_Click;
            //}
        }
        ...
        private void BtnUpdate_Click(object sender, EventArgs e)
        {
            if(gridView1.FocusedRowHandle >= 0)
            {
                ...
                List<string> ListBatch = CJia.PIVAS.Common.BatchHandle(batchs);
        ...
        public void ExeinitData(DataTable dt)
        {
            if(!dt.IsNull())
            {
                if(User.Role == "0")
                {
                    dt = CJia.PIVAS.Tools.Helper.GetDataSource(dt.Select(" ILLFIELD_ID = '" + User.DeptId + "' "));
                }
                if(!dt.IsNull())
                {
                    dt = CJia.PIVAS.Tools.Helper.GetDataSource(dt.Select(" FREQUENCY_ID <> '9999999999' "));
                }
            }
