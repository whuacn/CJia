...
namespace CJia.PIVAS.App
{
    static class Program
    {
        ...
        static void Application_ThreadException(object sender, System.Threading.ThreadExceptionEventArgs e)
        {
            ...
            CJia.PIVAS.Models.PIVASModel.WriteLog("异常信息：" + e.Exception.Source + e.Exception.TargetSite + e.Exception.Message);
        }
        ...
        private static bool SystemInitConfig()
        {
            try
            {
                ...
                {
                    Message.Show("配置文件配置错误，数据库访问异常。");
                    return false;
                }
            }
            catch(Exception ex)
            {
                Message.Show("配置文件配置错误，错误原因:" + ex.Message + "。");
                return false;
            }
        }
