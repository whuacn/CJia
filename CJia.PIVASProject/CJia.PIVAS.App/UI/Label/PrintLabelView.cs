    ...
    public partial class PrintLabelView : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        private void btnEnter_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if(this.rbnAll.Checked)
                {
                    if(MessageBox.Show("现在开始打印显示的所有的(" +  this.LabelDetail.Rows.Count.ToString() + ")张瓶贴！", "确认开始打印瓶贴", MessageBoxButtons.OKCancel) == DialogResult.OK)
                    {
                        this.IsPrint = true;
                        this.StartNum = 1;
                ...
                else if(this.rbnNoAll.Checked)
                {
                    ...
                    if(start > end)
                    {
                        MessageBox.Show("开始瓶贴号不能大于结束瓶贴号！");
                        IsSucceed = false;
                    }
                    if(end > this.LabelDetail.Rows.Count)
                    {
                        MessageBox.Show("结束瓶贴号不能大于总瓶贴数！");
                        IsSucceed = false;
                    }
                    if(IsSucceed)
                    {
                        if(MessageBox.Show("现在开始打印 " + start.ToString() + "号到" + end.ToString() + "号瓶贴", "确认开始打印瓶贴", MessageBoxButtons.OKCancel) == DialogResult.OK)
                        {
                            this.IsPrint = true;
                            this.StartNum = start;
                            this.EndNum = end;
